...
def find_threshold_rect(x, y, labels):
    n = x.size
    N1 = np.sum(labels)  # число единиц
    N0 = n - N1  # число нулей

    grid_x = np.linspace(np.min(x), np.max(x), 100, endpoint=False)
    grid_y = np.linspace(np.min(y), np.max(y), 100, endpoint=False)
    min_cost = None
    optimal_cutoff_x = None
    optimal_cutoff_y = None
    for cutoff_x, cutoff_y in zip(grid_x, grid_y):
        cost = 0.0
        for i in range(n):
            if labels[i] == 1:
                if not (x[i] >= cutoff_x and y[i] >= cutoff_y):
                    cost += (max(cutoff_x - x[i], 0) + max(cutoff_y - y[i], 0)) / N1
            else:  # labels[i] == 0
                if x[i] >= cutoff_x and y[i] >= cutoff_y:
                    cost += ((x[i] - cutoff_x) + (y[i] - cutoff_y)) / N0
        if min_cost is None or cost < min_cost:
            min_cost = cost
            optimal_cutoff_x = cutoff_x
            optimal_cutoff_y = cutoff_y

    labels_pred = np.zeros_like(labels, dtype=int)
    for i in range(n):
        if x[i] >= optimal_cutoff_x and y[i] >= optimal_cutoff_y:
            labels_pred[i] = 1
    auc = sklearn_metrics.roc_auc_score(labels, labels_pred)

    return optimal_cutoff_x, optimal_cutoff_y, auc
