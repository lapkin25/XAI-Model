...
cutpoints = [70, 82, 3, 135.7, 45, 75.6, 0.481, 0.24, 6.83, 115.0]

inverted_predictors = ["EF LV", "EOS", "SBP"]

F_list = []
for i in range(len(predictors)):
    name = 'F' + str(i + 1)
    pr = predictors[i]
    if pr not in inverted_predictors:
        df.loc[df[pr] >= cutpoints[i], (name)] = 1
        df.loc[df[pr] < cutpoints[i], (name)] = 0
    else:
        df.loc[df[pr] <= cutpoints[i], (name)] = 1
        df.loc[df[pr] > cutpoints[i], (name)] = 0
    F_list.append(name)
...
for i in range(len(predictors)):
    for j in range(i + 1, len(predictors)):
        name = 'G' + str(i + 1) + "_" + str(j + 1)
        pr1 = predictors[i]
        pr2 = predictors[j]
        if pr1 not in inverted_predictors and pr2 not in inverted_predictors:
            df.loc[(df[pr1] >= cutpoints[i]) & (df[pr2] >= cutpoints[j]), (name)] = 1
            df.loc[(df[pr1] < cutpoints[i]) | (df[pr2] < cutpoints[j]), (name)] = 0
        elif pr1 in inverted_predictors and pr2 in inverted_predictors:
            df.loc[(df[pr1] <= cutpoints[i]) & (df[pr2] <= cutpoints[j]), (name)] = 1
            df.loc[(df[pr1] > cutpoints[i]) | (df[pr2] > cutpoints[j]), (name)] = 0
        elif pr1 not in inverted_predictors and pr2 in inverted_predictors:
            df.loc[(df[pr1] >= cutpoints[i]) & (df[pr2] <= cutpoints[j]), (name)] = 1
            df.loc[(df[pr1] < cutpoints[i]) | (df[pr2] > cutpoints[j]), (name)] = 0
        elif pr1 in inverted_predictors and pr2 not in inverted_predictors:
            df.loc[(df[pr1] <= cutpoints[i]) & (df[pr2] >= cutpoints[j]), (name)] = 1
            df.loc[(df[pr1] > cutpoints[i]) | (df[pr2] < cutpoints[j]), (name)] = 0
        G_list.append(name)
...
for i in range(len(predictors)):
    for j in range(i + 1, len(predictors)):
        for k in range(j + 1, len(predictors)):
            name = 'H' + str(i + 1) + "_" + str(j + 1) + "_" + str(k + 1)
            pr1 = predictors[i]
            pr2 = predictors[j]
            pr3 = predictors[k]
            if pr1 not in inverted_predictors:
                RF1 = (df[pr1] >= cutpoints[i])
            else:
                RF1 = (df[pr1] <= cutpoints[i])
            if pr2 not in inverted_predictors:
                RF2 = (df[pr2] >= cutpoints[j])
            else:
                RF2 = (df[pr2] <= cutpoints[j])
            if pr3 not in inverted_predictors:
                RF3 = (df[pr3] >= cutpoints[k])
            else:
                RF3 = (df[pr3] <= cutpoints[k])
            df.loc[RF1 & RF2 & RF3, (name)] = 1
            df.loc[~RF1 | ~RF2 | ~RF3, (name)] = 0
            H_list.append(name)

#features = F_list + G_list + H_list
#features = F_list + G_list
#features = G_list
features = F_list


isModel = 1  # 1 - Logistic,  10 - Lasso
rm = 100
border = 0.03
np.random.seed(rm)
...
for j in range(100):
    ...
    if isModel == 1:
        ...
        model = LogisticRegression(solver='liblinear', max_iter=max_iter1, C=lasso_alpha, penalty='l1')
    else:
