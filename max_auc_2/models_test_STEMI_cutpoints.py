...
# Пороги:
# Возраст ≥67.6141750939809
# NER1 ≥2851.92356663509
# SIRI ≥6.675510789705735
# СОЭ ≥35.46879380317242
# TIMI после ≤1.8996069134161897
# СДЛА ≥35.52387717581928
# Killip ≥3.159671820862726
# RR 600-1200 ≥870.3493540805769
# интервал PQ 120-200 ≤197.57468555271487
# RR 600-1200_ ≤681.4175134782593

# Фенотипы:
# Возраст
# NER1 & (SIRI | СОЭ | TIMI после | СДЛА | Killip | RR 600-1200 | интервал PQ 120-200 | RR 600-1200_)
# SIRI
# СОЭ
# TIMI после
# СДЛА & (интервал PQ 120-200)
# Killip
# RR 600-1200 & (Возраст | NER1 | SIRI | СОЭ | TIMI после | СДЛА | Killip | интервал PQ 120-200 | RR 600-1200_)
# интервал PQ 120-200 & (Возраст | SIRI | СОЭ | TIMI после | СДЛА | Killip)
# RR 600-1200_


# Возраст
df.loc[(df['Возраст'] >= 67.61), ('F1')] = 1
df.loc[(df['Возраст'] < 67.61), ('F1')] = 0

# NER1 & (SIRI | СОЭ | TIMI после | СДЛА | Killip | RR 600-1200 | интервал PQ 120-200 | RR 600-1200_)
df.loc[(df['NER1'] >= 2851.92) & ((df['SIRI'] >= 6.675) |
                                  (df['СОЭ'] >= 35.46) |
                                  (df['TIMI после'] <= 1.89) |
                                  (df['СДЛА'] >= 35.52) |
                                  (df['Killip'] >= 3.15) |
                                  (df['интервал PQ 120-200'] <= 197.57) |
                                  (df['RR 600-1200'] <= 681.41)), ('F2')] = 1
df.loc[(df['NER1'] < 2851.92) | ((df['SIRI'] < 6.675) &
                                  (df['СОЭ'] < 35.46) &
                                  (df['TIMI после'] > 1.89) &
                                  (df['СДЛА'] < 35.52) &
                                  (df['Killip'] < 3.15) &
                                  (df['интервал PQ 120-200'] > 197.57) &
                                  (df['RR 600-1200'] > 681.41)), ('F2')] = 0

# SIRI
df.loc[(df['SIRI'] >= 6.675), ('F3')] = 1
df.loc[(df['SIRI'] < 6.675), ('F3')] = 0

# СОЭ
df.loc[(df['СОЭ'] >= 35.46), ('F4')] = 1
df.loc[(df['СОЭ'] < 35.46), ('F4')] = 0

# TIMI после
df.loc[(df['TIMI после'] <= 1.89), ('F5')] = 1
df.loc[(df['TIMI после'] > 1.89), ('F5')] = 0

# СДЛА & (интервал PQ 120-200)
df.loc[(df['СДЛА'] >= 35.52) & (df['интервал PQ 120-200'] <= 197.57), ('F6')] = 1
df.loc[(df['СДЛА'] < 35.52) | (df['интервал PQ 120-200'] > 197.57), ('F6')] = 0

# Killip
df.loc[(df['Killip'] >= 3.15), ('F7')] = 1
df.loc[(df['Killip'] < 3.15), ('F7')] = 0

# RR 600-1200 & (Возраст | NER1 | SIRI | СОЭ | TIMI после | СДЛА | Killip | интервал PQ 120-200 | RR 600-1200_)
df.loc[(df['RR 600-1200'] >= 870.34) & ((df['Возраст'] >= 67.61) |
                                        (df['NER1'] >= 2851.92) |
                                        (df['SIRI'] >= 6.675) |
                                        (df['СОЭ'] >= 35.46) |
                                        (df['TIMI после'] <= 1.89) |
...
df.loc[(df['RR 600-1200'] < 870.34) | ((df['Возраст'] < 67.61) &
                                        ...
                                        (df['Killip'] < 3.15) &
                                        (df['интервал PQ 120-200'] > 197.57) &
                                        (df['RR 600-1200'] > 681.41)), ('F8')] = 0

# интервал PQ 120-200 & (Возраст | SIRI | СОЭ | TIMI после | СДЛА | Killip)
df.loc[(df['интервал PQ 120-200'] <= 197.57) & ((df['Возраст'] >= 67.61) |
                                                 (df['SIRI'] >= 6.675) |
                                                 (df['СОЭ'] >= 35.46) |
                                                 (df['TIMI после'] <= 1.89) |
                                                 (df['СДЛА'] >= 35.52) |
...
# RR 600-1200_
df.loc[(df['RR 600-1200'] <= 681.41), ('F10')] = 1
df.loc[(df['RR 600-1200'] > 681.41), ('F10')] = 0
