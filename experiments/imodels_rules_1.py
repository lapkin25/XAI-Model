...
# 3. Создание и обучение модели RuleFitClassifier
# Подробнее о параметрах модели можно узнать в документации imodels[reference:1]
model = RuleFitClassifier(
    #tree_size=4,  # Максимальная глубина деревьев: чем больше, тем сложнее правила
    tree_size=3,
    sample_fract='auto',  # Доля выборки для построения дерева ('auto' = 0.5 для классификации)
    max_rules=2000,  # Максимальное количество правил для рассмотрения
    #max_rules=100,
    memory_par=0.01,  # Параметр регуляризации для L1-штрафа (чем выше, тем меньше правил)
...
# 4. Извлечение правил (работает через защищённый метод _get_rules())
# Метод возвращает pandas.DataFrame со столбцами:
# rule     - текстовое описание правила
# type     - тип правила ('linear' для исходных признаков или 'rule' для составных)
# coef     - коэффициент правила (вклад в итоговое предсказание)
# support  - поддержка правила (доля обучающих объектов, для которых правило сработало)
# importance- важность правила
rules_df = model._get_rules(exclude_zero_coef=True)
# Отфильтруем только правила (исключим линейные признаки, оставим только составные)
rules_only = rules_df[rules_df['type'] == 'rule'].copy()
# Отсортируем по абсолютному значению коэффициента (по убыванию)
rules_only['abs_coef'] = rules_only['coef'].abs()
rules_only_sorted = rules_only.sort_values('abs_coef', ascending=False)

print("Извлеченные решающие правила (RuleFit):\n" + "="*60)
for idx, row in rules_only_sorted.head(10).iterrows():
    print(f"Правило: {row['rule']}")
    print(f"  Коэффициент: {row['coef']:.4f}  |  Поддержка: {row['support']:.2f}  |  Важность: {row['importance']:.4f}\n")
