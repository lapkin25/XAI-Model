...
# Age ≥70.8668362248291
# HR ≥86.3909811782295
# Killip class ≥2.9503693703495313
# Cr ≥190.61401504216627
# EF LV ≤32.94502429037709
# NEUT ≥76.01746130079343
# EOS ≤1.1857342054604232
# PCT ≥0.423508415623815
# Glu ≥7.68359033840621
# SBP ≤105.31622557539319

# Age & (NEUT | EOS)
# HR & (NEUT | EOS)
# Killip class & (NEUT | EOS)
# Cr & (EOS)
# EF LV
# NEUT & (EOS)
# EOS & (Age | NEUT)
...
# Age & (NEUT | EOS)
df.loc[(df['Age'] >= 70.8668) & ((df['NEUT'] >= 76.0174) | (df['EOS'] <= 1.1857)), ('F1')] = 1
df.loc[(df['Age'] < 70.8668) | ((df['NEUT'] < 76.0174) & (df['EOS'] > 1.1857)), ('F1')] = 0

# HR & (NEUT | EOS)
df.loc[(df['HR'] >= 86.390) & ((df['NEUT'] >= 76.0174) | (df['EOS'] <= 1.1857)), ('F2')] = 1
df.loc[(df['HR'] < 86.390) | ((df['NEUT'] < 76.0174) & (df['EOS'] > 1.1857)), ('F2')] = 0

# Killip class & (NEUT | EOS)
df.loc[(df['Killip class'] >= 2.950) & ((df['NEUT'] >= 76.0174) | (df['EOS'] <= 1.1857)), ('F3')] = 1
df.loc[(df['Killip class'] < 2.950) | ((df['NEUT'] < 76.0174) & (df['EOS'] > 1.1857)), ('F3')] = 0

# Cr & (EOS)
df.loc[(df['Cr'] >= 190.614) & (df['EOS'] <= 1.1857), ('F4')] = 1
df.loc[(df['Cr'] < 190.614) | (df['EOS'] > 1.1857), ('F4')] = 0

# EF LV
df.loc[(df['EF LV'] <= 32.9450), ('F5')] = 1
df.loc[(df['EF LV'] > 32.9450), ('F5')] = 0

# NEUT & (EOS)
df.loc[(df['NEUT'] >= 76.0174) & (df['EOS'] <= 1.1857), ('F6')] = 1
df.loc[(df['NEUT'] < 76.0174) | (df['EOS'] > 1.1857), ('F6')] = 0

# EOS & (Age | NEUT)
df.loc[(df['EOS'] <= 1.1857) & ((df['Age'] >= 70.8668) | (df['NEUT'] >= 76.0174)), ('F7')] = 1
df.loc[(df['EOS'] > 1.1857) | ((df['Age'] < 70.8668) & (df['NEUT'] < 76.0174)), ('F7')] = 0

# PCT & (EOS)
df.loc[(df['PCT'] >= 0.4235) & (df['EOS'] <= 1.1857), ('F8')] = 1
df.loc[(df['PCT'] < 0.4235) | (df['EOS'] > 1.1857), ('F8')] = 0

# Glu & (NEUT | EOS)
df.loc[(df['Glu'] >= 7.6835) & ((df['NEUT'] >= 76.0174) | (df['EOS'] <= 1.1857)), ('F9')] = 1
df.loc[(df['Glu'] < 7.6835) | ((df['NEUT'] < 76.0174) & (df['EOS'] > 1.1857)), ('F9')] = 0

# SBP & (EOS)
df.loc[(df['SBP'] <= 105.3162) & (df['EOS'] <= 1.1857), ('F10')] = 1
df.loc[(df['SBP'] > 105.3162) | (df['EOS'] > 1.1857), ('F10')] = 0
