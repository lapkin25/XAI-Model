...
df.loc[(df['NEUT'] >= 77.3), ('F1')] = 1
df.loc[(df['NEUT'] < 77.3), ('F1')] = 0

df.loc[(df['EOS'] < 0.2), ('F2')] = 1
df.loc[(df['EOS'] >= 0.2), ('F2')] = 0

df.loc[(df['Cr'] >= 122.95), ('F3')] = 1
df.loc[(df['Cr'] < 122.95), ('F3')] = 0

df.loc[(df['HR'] >= 85.0) & (df['NEUT'] >= 77.3), ('F4')] = 1
df.loc[(df['HR'] < 85.0) | (df['NEUT'] < 77.3), ('F4')] = 0

df.loc[(df['EOS'] < 0.2) & (df['NEUT'] >= 77.3), ('F5')] = 1
df.loc[(df['EOS'] >= 0.2) | (df['NEUT'] < 77.3), ('F5')] = 0

df.loc[(df['Cr'] >= 122.95) & (df['NEUT'] >= 77.3), ('F6')] = 1
df.loc[(df['Cr'] < 122.95) | (df['NEUT'] < 77.3), ('F6')] = 0

df.loc[(df['HR'] >= 85.0), ('F7')] = 1
df.loc[(df['HR'] < 85.0), ('F7')] = 0

df.loc[(df['Glu'] >= 7.62), ('F8')] = 1
df.loc[(df['Glu'] < 7.62), ('F8')] = 0

df.loc[(df['EF LV'] < 48.0), ('F9')] = 1
df.loc[(df['EF LV'] >= 48.0), ('F9')] = 0

df.loc[(df['Glu'] >= 7.62) & (df['NEUT'] >= 77.3), ('F10')] = 1
df.loc[(df['Glu'] < 7.62) | (df['NEUT'] < 77.3), ('F10')] = 0

df.loc[(df['Age'] >= 71), ('F11')] = 1
df.loc[(df['Age'] < 71), ('F11')] = 0
...
for feat in features:
    ...
    for j in range(100):
        ...
        x_train, x_validate, y_train, y_validate = train_test_split(x_feat, y1, train_size=0.8, stratify=y1,
                                                                    random_state=j + 42)
        model = LogisticRegression(solver=solver1, max_iter=max_iter1, C=C1)
