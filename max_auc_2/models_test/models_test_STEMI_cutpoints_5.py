...
# Пороги:
# Возраст ≥66.04341879663556
# NER1 ≥1936.2148136286564
# SIRI ≥5.519446118973876
# СОЭ ≥32.81828508038829
# TIMI после ≤1.6041904826542923
# СДЛА ≥24.608691428220055
# Killip ≥3.259975435445046
# RR 600-1200 ≥1270.2423264953372
# интервал PQ 120-200 ≤108.9846215124038
# RR 600-1200_ ≤570.4237958326273

# Фенотипы:
# Возраст & (СДЛА | интервал PQ 120-200)
# NER1 & (СОЭ)
# SIRI
# СОЭ & (СДЛА | RR 600-1200_)
# TIMI после
# СДЛА & (Возраст | SIRI | интервал PQ 120-200 | TIMI после)
# Killip & (СДЛА)
# RR 600-1200 & (Возраст)
# интервал PQ 120-200
# RR 600-1200_

# Веса: [ 0.47796275 -0.00654854  0.74479866  0.67343889  0.62734328  0.9550933
#   1.22827338  0.          0.67748172  0.7539866 ]


# Возраст & (СДЛА | интервал PQ 120-200)
df.loc[(df['Возраст'] >= 66.043) & ((df['СДЛА'] >= 24.6086) |
                                    (df['интервал PQ 120-200'] <= 108.9846)), ('F1')] = 1
df.loc[(df['Возраст'] < 66.043) | ((df['СДЛА'] < 24.6086) &
                                    (df['интервал PQ 120-200'] > 108.9846)), ('F1')] = 0

# SIRI
df.loc[(df['SIRI'] >= 5.51944), ('F2')] = 1
df.loc[(df['SIRI'] < 5.51944), ('F2')] = 0

# СОЭ & (СДЛА | RR 600-1200_)
df.loc[(df['СОЭ'] >= 32.81828) & ((df['СДЛА'] >= 24.6086) |
                                  (df['RR 600-1200'] <= 570.4237)), ('F3')] = 1
df.loc[(df['СОЭ'] < 32.81828) | ((df['СДЛА'] < 24.6086) &
                                  (df['RR 600-1200'] > 570.4237)), ('F3')] = 0

# TIMI после
df.loc[(df['TIMI после'] <= 1.6041), ('F4')] = 1
df.loc[(df['TIMI после'] > 1.6041), ('F4')] = 0

# СДЛА & (Возраст | SIRI | интервал PQ 120-200 | TIMI после)
df.loc[(df['СДЛА'] >= 24.608) & ((df['Возраст'] >= 66.043) |
                                 (df['SIRI'] >= 5.519) |
                                 (df['интервал PQ 120-200'] <= 108.9846) |
                                 (df['TIMI после'] <= 1.6041)), ('F5')] = 1
df.loc[(df['СДЛА'] < 24.608) | ((df['Возраст'] < 66.043) &
                                 (df['SIRI'] < 5.519) &
                                 (df['интервал PQ 120-200'] > 108.9846) &
                                 (df['TIMI после'] > 1.6041)), ('F5')] = 0

# Killip & (СДЛА)
df.loc[(df['Killip'] >= 3.2599) & (df['СДЛА'] >= 24.6086), ('F6')] = 1
df.loc[(df['Killip'] < 3.2599) | (df['СДЛА'] < 24.6086), ('F6')] = 0

# интервал PQ 120-200
df.loc[(df['интервал PQ 120-200'] <= 108.9846), ('F7')] = 1
df.loc[(df['интервал PQ 120-200'] > 108.9846), ('F7')] = 0

# RR 600-1200_
df.loc[(df['RR 600-1200'] <= 570.4237), ('F8')] = 1
df.loc[(df['RR 600-1200'] > 570.4237), ('F8')] = 0
